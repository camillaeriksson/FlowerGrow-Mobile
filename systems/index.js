// File that holds all the physics for all entities
import Physics from './physics';
import BadCloudPhysics from './BadCloudPhysics';
import FlowerPhysics from './FlowerPhysics';
import GroundPhysics from './GroundPhysics';
import WaterMeterPhysics from './WaterMeterPhysics';
import GoodCloudPhysics from './GoodCloudPhysics';
import StemPhysics from './StemPhysics';
import BeePhysics from './BeePhysics';
export default [Physics, BadCloudPhysics, BeePhysics, FlowerPhysics, WaterMeterPhysics, GoodCloudPhysics, GroundPhysics, StemPhysics];
